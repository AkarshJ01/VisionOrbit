...
def weighted_obb_fusion(detections, iou_threshold=0.60, min_area=12.0):
    """
    Class-agnostic weighted OBB fusion.

    Overlapping detections are treated as the same physical object even when
    the model predicts different classes for them. Geometry is fused using
    confidence-weighted polygon averaging, while the final class is selected
    using accumulated confidence evidence.
    """
    if len(detections) == 0:
        return []
    ...
    for item in valid:
        ...
        for cluster in clusters:
            iou = polygon_iou(poly, cluster["fused_poly"])
        ...
        if matched_cluster is None:
            clusters.append({
                "items": [item],
                "fused_poly": poly.copy(),
            })
            continue

        matched_cluster["items"].append(item)

        items = matched_cluster["items"]

        weights = np.array(
            [x["conf"] for x in items],
            dtype=np.float32
        )
        ...
        all_polys = np.array(
            [x["poly"] for x in items],
            dtype=np.float32
        )

        matched_cluster["fused_poly"] = np.sum(
            all_polys * weights[:, None, None],
            axis=0
        )
    ...
    for cluster in clusters:
        ...
        # Accumulate confidence by class. This allows multiple tile/global
        # predictions to vote for the final class.
        class_scores = {}

        for item in items:
            cid = item["cls_id"]
            class_scores[cid] = class_scores.get(cid, 0.0) + float(item["conf"])

        winning_cid = max(class_scores, key=class_scores.get)

        winning_items = [
            x for x in items
            if x["cls_id"] == winning_cid
        ]
        ...
        # Use the strongest confidence rather than artificially inflating
        # confidence simply because multiple predictions overlap.
        final_conf = max(
            float(x["conf"])
            for x in items
        )
        ...
        final_dets.append({
            "cls_id": winning_cid,
            "cls_name": winning_item["cls_name"],
            "conf": round(final_conf, 4),
            "poly": cluster["fused_poly"],
        })
